...
class SafetyCultureClient:
    ...
    def get_template_by_id(self, template_id):
        try:
            response = self._make_request(f"{self.base_url}/templates/v1/templates/{template_id}")
    ...
    for permission_type, permission_list in permissions.items():
        ...
        for permission_entry in permission_list:
            ...
            if permission_obj_type == 'ROLE':
                assignee_type, assignee_name = 'group', groups_lookup.get(assignee_id, f'Unknown Group ({assignee_id})')
            else:
                assignee_type, assignee_name = 'user', users_lookup.get(assignee_id, f'Unknown User ({assignee_id})')
            records.append({
                'template_id': template_id, 'name': template_name, 'permission': permission_type,
                'assignee_type': assignee_type, 'assignee_id': assignee_id, 'assignee_name': assignee_name
            })
    return records
...
def main():
    ...
    with open('template_access_rules.csv', 'w', newline='', encoding='utf-8') as csv_file:
        csv_writer = csv.writer(csv_file)
        csv_writer.writerow(['template_id', 'name', 'permission', 'assignee_type', 'assignee_id', 'assignee_name'])
        ...
        for template_summary in client.fetch_paginated_feed('/feed/templates'):
            ...
            if template_detail:
                for record in process_template_permissions(template_detail, users_lookup, groups_lookup):
                    csv_writer.writerow([record['template_id'], record['name'], record['permission'],
                                       record['assignee_type'], record['assignee_id'], record['assignee_name']])
