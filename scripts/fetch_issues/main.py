...
class IssuesExtractor:  # pylint: disable=too-many-instance-attributes
    ...
    async def retry_async_call(self, func, *args, **kwargs):
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                print(f"Async API call failed (attempt {attempt + 1}/{self.max_retries}): {e}")
    ...
    async def fetch_feed_chain(
        ...
        print(f"Completed {endpoint}: {len(all_data)} total items in {page_count} pages")
        return all_data
    ...
    def find_completion_user(self, task_id: str) -> tuple[Optional[str], Optional[str]]:
        ...
        status_events = [
            item for item in self.timeline_data
            if (item.get("task_id") == task_id and
                item.get("item_type") == "TASK_STATUS_UPDATED")
        ]
    ...
    def get_assignee_names(self, task_id: str) -> str:
        ...
        task_assignees = [
            item for item in self.assignees_data
            if item.get("issue_id") == task_id
        ]
    ...
    def process_issues_to_csv(self) -> pd.DataFrame:
        ...
        for issue in self.issues_data:
            try:
                ...
                completion_user_id, completion_user_name = self.find_completion_user(task_id)
    ...
    def export_raw_feeds_to_csv(self) -> str:
        ...
        if self.timeline_data:
            ...
            print(f"Exported {len(timeline_df)} raw timeline items to: raw_timeline_items.csv")
